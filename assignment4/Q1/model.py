...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            cov_3D = torch.matmul(quats_to_rotmat(quats), scales.unsqueeze(-1) * torch.eye(3).unsqueeze(0).repeat(len(scales), 1, 1))
    ...
    def compute_cov_2D(
        ...
        cov_2D = torch.matmul(torch.matmul(J, cov_3D), torch.transpose(J, 1, 2))  # (N, 2, 2)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        z_vals = self.gaussians.means_3D[:, 2]  # (N,)
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        # Calculate camera origin in the world frame
        camera_origin = camera.get_camera_center()  # shape (1, 3)
        # Calculate direction vectors from camera origin to each 3D Gaussian
        gaussian_dirs = means_3D - camera_origin  # shape (N, 3)       
        # Normalize the computed directions
